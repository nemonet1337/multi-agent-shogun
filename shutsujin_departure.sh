...
#   ./shutsujin_departure.sh -k        # 決戦の陣（全足軽Opus）
#   ./shutsujin_departure.sh -s        # セットアップのみ（Claude起動なし）
#   ./shutsujin_departure.sh -g        # Gemini CLI で全エージェント起動
#   ./shutsujin_departure.sh -x        # Codex CLI で全エージェント起動
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  -g, --gemini        Gemini CLI を使用（試験的機能・全エージェントGemini）"
            echo "  -x, --codex         Codex CLI を使用（試験的機能・全エージェントCodex）"
            ...
            echo ""
            echo "例:"
            echo "  ./shutsujin_departure.sh              # 前回の状態を維持して出陣（Claude）"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 注: Upstreamとの互換性のため、-g/-x 以外はメニューを出さずにデフォルト(Claude)で進む
if [ -n "$AGENT_TYPE" ]; then
    log_info "  └─ 指定されたエージェントを使用: $AGENT_TYPE"
else
    # デフォルト動作（Claude）
    # メニューは廃止（Upstream準拠）
    :
...
# エージェント別コマンド設定（HEAD互換・グローバルオーバーライド用）
if [ "$AGENT_TYPE" = "codex" ]; then
    ...
    # Default (Claude) - will use Upstream's logic later
...
if [ "$SETUP_ONLY" = false ]; then
    ...
    if [ -n "$AGENT_TYPE" ]; then
        # =======================================================================
        # HEAD Global Override Logic (Gemini/Codex)
        # =======================================================================
        
        # エージェント CLI の存在チェック (HEAD style)
        if [ "$AGENT_TYPE" = "codex" ]; then
        ...
        # 家老・足軽・軍師（9ペイン）
        # HEAD had 0..8 (9 panes). Upstream layout is also 9 panes.
        for i in {0..8}; do
            p=$((PANE_BASE + i))
            tmux send-keys -t "multiagent:agents.${p}" "$AGENT_CMD"
            tmux send-keys -t "multiagent:agents.${p}" Enter
        done
        log_info "  └─ 家老・足軽・軍師、召喚完了"
        log_success "✅ 全軍 ${AGENT_DISPLAY_NAME} 起動完了"
        
        # HEADの指示書読み込みロジック (マニュアル送信)
        # Geminii/CodexはCLAUDE.mdを自動読み込みしない可能性があるため
        log_war "📜 各エージェントに指示書を読み込ませ中..."
        
        echo "  ${AGENT_DISPLAY_NAME} の起動を待機中（最大30秒）..."
        for i in {1..30}; do
            # 起動したかチェック (prompt文字列などで判断したいが、Geminiはprompt違うかも)
            # とりあえず sleep
            sleep 1
            # 簡易チェック: pane captureして起動待ち判定できれば良いが、汎用的に難しいので単純ウェイト
            # HEADは 'bypass permissions' を待っていた (Claude用)
            # Gemini/Codexの場合は即時でもいいかもしれない
            if [ "$AGENT_TYPE" != "claude" ]; then
                # Gemini/Codexは早いと仮定して5秒でbreak
        ...
        # 足軽 (1-7) & 軍師 (as Ashigaru 8 or Gunshi?)
        # Upstream architecture uses pane 8 as Gunshi.
        # HEAD loop was 1..8. 
        # With AGENT_TYPE override, we just treat them all as workers + Gunshi.
        sleep 2
        log_info "  └─ 足軽・軍師に指示書を伝達中..."
        for i in {1..7}; do
            p=$((PANE_BASE + i))
        ...
        # Gunshi (Pane 8)
        p=$((PANE_BASE + 8))
        tmux send-keys -t "multiagent:agents.${p}" "instructions/shogun.md を読んで役割を理解せよ（軍師として）" 
        # (Wait, Gunshi instruction?) Upstream doesn't have instructions/gunshi.md yet?
        # HEAD was using ashigaru.md for 1..8.
        # I'll use instructions/ashigaru.md for Gunshi too if specialized doc missing, or shogun.md
        # Upstream relies on CLAUDE.md.
        # I'll send "instructions/shogun.md" to Gunshi as fallback for now.
        tmux send-keys -t "multiagent:agents.${p}" Enter
        ...
        # =======================================================================
        # UPSTREAM Standard Logic (Claude / CLI Adapter)
        # =======================================================================
